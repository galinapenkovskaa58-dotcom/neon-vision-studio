...
import { z } from 'zod';
import { Send, CheckCircle, MessageCircleQuestion, Sparkles, Rocket } from 'lucide-react';
...
export type RequestVariant = 'question' | 'project' | 'booking';
...
const variants: Record<RequestVariant, VariantConfig> = {
  question: {
    icon: MessageCircleQuestion,
    title: 'Задать вопрос',
    subtitle: 'Расскажите, что вас интересует — мы ответим в выбранном мессенджере',
    fieldLabel: 'Ваш вопрос *',
    fieldPlaceholder: 'Опишите, что вы хотите узнать...',
    successTitle: 'Вопрос отправлен!',
    successText: 'Мы свяжемся с вами в ближайшее время и ответим на все вопросы.',
  ...
  project: {
    ...
    fieldLabel: 'Расскажите о проекте *',
    fieldPlaceholder: 'Что хотите создать? Идея, цели, формат, сроки...',
    successTitle: 'Заявка отправлена!',
    successText: 'Скоро свяжемся, обсудим детали и предложим решение.',
  ...
  booking: {
    ...
    fieldLabel: 'Что хотите заказать? *',
    fieldPlaceholder: 'Опишите услугу, формат, пожелания...',
...
const schema = z.object({
  text: z.string().trim().min(5, 'Опишите подробнее').max(2000, 'Слишком длинный текст'),
...
export default function RequestDialog({ open, onOpenChange, variant = 'question' }: RequestDialogProps) {
  ...
  const mutation = useMutation({
    mutationFn: async (data: FormData) => {
      ...
      const tag = `[${data.messenger.toUpperCase()}] ${data.contact}`;
      ...
      if (variant === 'question') {
        const { error } = await supabase.from('questions').insert({
          ...
          messenger_username: tag,
          question: data.text,
        });
        if (error) throw error;
      } else {
        // project & booking → bookings table
        const prefix = variant === 'project' ? '[ПРОЕКТ] ' : '[ЗАЯВКА] ';
        const { error } = await supabase.from('bookings').insert({
          name: data.name,
          phone,
          messenger: dbMessenger,
          messenger_username: tag,
          references_text: prefix + data.text,
        });
        if (error) throw error;
      }
    },
  ...
  const handleOpenChange = (next: boolean) => {
    if (!next) {
      setTimeout(() => {
        setSubmitted(false);
        setErrors({});
        setForm({ text: '', name: '', messenger: 'telegram', contact: '', consent: false });
      }, 200);
    }
    onOpenChange(next);
  };
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto glass-strong border-border/40 rounded-3xl p-0">
        {submitted ? (
          ...
          >
            <DialogTitle className="sr-only">{config.successTitle}</DialogTitle>
            <DialogDescription className="sr-only">{config.successText}</DialogDescription>
            <CheckCircle size={64} className="text-neon-cyan mx-auto mb-6" />
            <h3 className="text-2xl font-heading font-bold mb-3">{config.successTitle}</h3>
            <p className="text-muted-foreground">{config.successText}</p>
          </motion.div>
        ) : (
          <div className="p-8 md:p-10">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label className="block text-sm font-medium mb-2">{config.fieldLabel}</label>
                <Textarea
                  value={form.text}
